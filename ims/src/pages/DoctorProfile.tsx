        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="flex items-center gap-4">
            <div className="h-20 w-20 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden">
              {doctor?.avatar ? <img src={doctor.avatar} alt="avatar" className="h-full w-full object-cover" /> : <div className="text-2xl text-gray-600">{doctor?.name ? doctor.name.charAt(0).toUpperCase() : "D"}</div>}
            </div>
            <div>
              <div className="text-sm text-gray-500">Doctor Profile</div>
              <div className="text-2xl font-semibold text-gray-900">{doctor?.name || "Doctor"}</div>
